...
- **误区：测好了就能全量发。** 多数故障源于变更，应灰度把影响面从全站缩到一小撮，可控可回滚。
- **误区：灰度靠人肉盯盘放行。** 应自动门禁：错误率/延迟/资源与基线对比，越线即阻断/回滚。
- **误区：数据库变更能直接回滚。** 需 expand/contract 两阶段保持新旧兼容，破坏性 DDL 与发布解耦。
- **误区：加列同时删旧列。** 回滚后旧代码找不到列会崩，必须分阶段（先扩展后收缩）。
- **误区：出故障先查根因。** 变更是主要故障源，回滚是最快止血，根因事后再查。
