...
> **摘要**：风控系统在关键业务动作（登录、下单、支付、提现、发帖）发生的**毫秒级**内，判断该行为是否欺诈/作弊/异常并做出放行、拦截或加验的决策。核心是**实时决策链路 + 规则与模型协同 + 名单/画像 + 攻防对抗**。本文按「决策链路 → 规则引擎 → 特征与名单 → 规则与模型协同 → 误杀漏杀权衡 → 对抗 → 面试问答」推进。
...
- **风控为什么用规则引擎？** 对抗变化快，规则要可配置、秒级生效与回滚，不能靠发版。
- **规则和模型怎么分工？** 规则处理明确可解释策略，模型处理复杂隐性模式，二者互补。
- **误杀和漏杀怎么权衡？** 调阈值 + 中间「加验」档：资金从严、体验从宽。
- **怎么识别团伙欺诈？** 关联图谱把用户-设备-IP 连成图，看群体异常。
- **风控系统挂了怎么办？** 默认策略兜底：核心资金从严、普通链路放行，避免阻断全站。
- **怎么持续提升？** 样本回流闭环，实时拦截 + 离线挖掘。
