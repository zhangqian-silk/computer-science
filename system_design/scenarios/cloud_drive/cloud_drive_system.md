...
> **摘要**：云盘的核心是**大文件可靠上传下载**与**元数据一致性**。关键能力：分片上传、断点续传、秒传（去重）、元数据与对象存储分离、分享与回收站。本文按「架构分层 → 分片上传与断点续传 → 秒传去重 → 元数据一致性 → 下载与分享 → 成本 → 面试问答」推进。
...
- **大文件怎么上传？** 分片上传 + 并发 + 断点续传 + 合并提交（幂等）。
- **秒传怎么实现？** 内容哈希去重 + 引用计数，命中则只建元数据引用。
- **秒传有什么风险？** 哈希碰撞（强哈希+大小校验）、越权（须权限校验）。
- **元数据和内容怎么保持一致？** 先传内容再提交元数据，提交前不可见，失败幂等重试。
- **下载为什么用签名 URL？** 客户端直连对象存储/CDN 省带宽，限时防盗链。
- **怎么降成本？** 去重压缩 + 冷热分层 + 生命周期迁移。
