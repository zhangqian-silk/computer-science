...
> **摘要**：支付系统的第一诉求是**资金零差错**——不能多扣、少扣、重复扣、丢单。它把「交易一致性、幂等防重、记账、对账、通道治理」拧成一个对错误零容忍的链路。本文按「主链路 → 幂等与状态机 → 复式记账 → 对账清结算 → 通道路由与高可用 → 指标 → 关键设计要点」推进。
...
- **支付结果口径**：以通道异步回调为准，且回调会重发，处理必须幂等。
- **防重复扣款**：幂等键（商户订单号）+ 状态机终态幂等。
- **复式记账**：借贷恒等（账永远平），是对账的数学保证。
- **对账**：发现并修正异步链路的不一致，是资金安全最后防线。
- **回调丢失**：主动向通道查询补偿（回查）+ 消息重试，双保险。
