...
> **摘要**：推荐系统从海量物品中为每个用户选出最可能感兴趣的少量内容。它与搜索、广告共享「多路召回 → 排序漏斗」骨架，但更强调**个性化、实时性、冷启动与多样性**。本文按「链路 → 多路召回 → 排序 → 特征与训练服务一致 → 冷启动与探索 → 多样性 → 关键设计要点」推进。
...
- **与搜索的区别**：搜索有明确 query，推荐用画像 + 上下文代替，更强调个性化与探索。
- **多路召回**：单路覆盖不全，多路互补再合并提高召回。
- **向量召回**：用户/物品编码成向量 + ANN 近邻检索，捕捉语义。
- **训练服务一致**：特征口径不一致会使线上效果远差于离线，需统一特征平台。
- **冷启动**：属性/热门兜底 + 探索流量 + EE 策略。
- **探索的必要性**：纯利用导致信息茧房，探索发现新兴趣、扶持新内容。
- **多目标**：只优化 CTR 会催生标题党，需多目标（时长/转化/留存）平衡。
