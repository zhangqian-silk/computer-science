...
> **摘要**：电商交易把「商品 → 库存 → 订单 → 支付 → 履约 → 售后」串成端到端链路，核心难点是**库存不超卖、订单状态一致、秒杀高并发、支付资金安全**。本文按「业务对象与主链路 → 库存与超卖 → 订单状态机 → 秒杀削峰 → 售后逆向 → 指标 → 面试问答」推进，是通用机制与组件在真实业务的组合落地。
...
- **怎么防超卖？** DB 原子扣减（`stock>0`）或 Redis 预扣减 + 对账；杜绝先查后改。
- **秒杀怎么扛？** 层层削峰：前端→限流防刷→Redis 预扣→异步下单→售罄快返。
- **未支付订单怎么处理？** 定时任务超时关单并回补库存。
- **下单跨服务一致性怎么保证？** Saga 编排 + 补偿 + 对账，状态机约束。
- **热点 SKU 扣减瓶颈？** 库存分桶打散热点行。
- **重复支付/发货怎么防？** 状态机 + 幂等键。
