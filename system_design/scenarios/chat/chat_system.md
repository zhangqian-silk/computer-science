...
> **摘要**：IM 要在**海量长连接**下做到消息**不丢、不重、有序、多端同步**，并区分单聊与群聊、在线与离线。核心难点是长连接接入层、消息可靠投递语义、写扩散/读扩散选择与群消息放大。本文按「架构分层 → 长连接接入 → 消息可靠与有序 → 单聊/群聊存储 → 写扩散vs读扩散 → 在线状态与多端 → 面试问答」推进。
...
- **消息怎么保证不丢不重？** 落库 + ack 重推（至少一次）+ 客户端按消息 ID 去重。
- **怎么保证有序？** 会话内单调 seq，客户端按 seq 排序补齐；不追求全局有序。
- **群聊写扩散还是读扩散？** 小群推、大群/大 V 拉或推拉结合。
- **海量长连接怎么扛？** 多网关水平扩展 + 连接路由表 + 心跳保活。
- **离线消息怎么处理？** 存离线队列/收件箱，上线按 seq 增量拉取。
- **多端已读怎么同步？** 会话 seq + 每端已读位点。
