...
> **摘要**：预订系统的核心是**有限资源的库存锁定与防超卖**（房间、座位、时段），叠加**支付前的预占与超时释放**、**支付回调乱序处理**。与电商秒杀相通，但资源更「独占」（一个座位只能给一个人）。本文按「资源模型 → 预占与超时释放 → 防超卖 → 状态机与乱序回调 → 高并发抢购 → 关键设计要点」推进。
...
- **与电商库存的差异**：有较长「预占→支付」窗口，需冻结 + 超时释放；座位型需精确锁定具体资源。
- **防超卖**：原子扣减/座位 CAS，杜绝先查后改。
- **超时回收**：定时任务/时间轮释放冻结资源。
- **回调晚于超时释放**：幂等 + 状态校验，已释放则退款而非确认。
- **热门票抗压**：排队令牌 + 限流 + Redis 预扣 + 座位短锁。
- **回调重复**：状态机终态幂等忽略。
