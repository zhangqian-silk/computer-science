...
> **摘要**：授权解决「你能做什么」，在认证成功后决定用户对资源/操作的权限。本文覆盖 RBAC、ABAC、ACL、OAuth 等模型，并补充模型选型、权限校验位置、数据权限（行/列级）、常见误区与引用。

> **前置阅读**：[认证](./authentication.md)先确认「你是谁」，本文再决定「你能做什么」；OAuth 的令牌与[服务身份](./service_identity.md)相关。
...
- **选型**：**RBAC**（角色）适合权限相对固定、按岗位划分的后台系统，简单易管；**ABAC**（属性/策略）适合需按「用户属性 + 资源属性 + 环境」动态判定的复杂场景（如「本部门经理在工作时间可审批 ≤1 万的单」），灵活但策略难维护；**ACL** 适合资源粒度的直接授权。多数系统以 RBAC 为主、ABAC 补充。
- **校验位置**：网关做粗粒度（能否访问某接口），服务内做细粒度（能否操作某条数据）。
- **数据权限（行/列级）**：不仅控「能否调接口」，还要控「能看哪些行、哪些列」——如销售只看自己客户。通常在查询层注入数据范围条件或用行级安全策略。

---

## 常见误区与澄清

- **误区：授权只在网关做。** 网关只能做粗粒度；「能否操作这条数据」的细粒度必须在服务内校验。
- **误区：RBAC 万能。** 角色爆炸或需按属性动态判定时，RBAC 难维护，应引入 ABAC。
- **误区：前端隐藏按钮就是授权。** 前端隐藏只是体验，后端必须校验，否则可绕过。
- **误区：只控接口不控数据。** 需行/列级数据权限，否则越权看到他人数据。
- **误区：OAuth 是认证协议。** OAuth 是授权框架（拿令牌访问资源），认证要用 OIDC（基于 OAuth 之上）。

---

## 引用关系

- 配合：[认证](./authentication.md)、[服务身份与密钥](./service_identity.md)
- 应用：后台权限系统、API 网关鉴权
