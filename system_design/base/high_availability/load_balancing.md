...
> **摘要**：负载均衡把流量分配到多个后端，优化资源、提升性能、避免单点。本文覆盖负载均衡类型（L4/L7、服务端/客户端）、算法（轮询/哈希/最少连接/响应时间/P2C）、健康检查与实例摘除、慢启动预热、异常检测、全局负载均衡（GSLB），以及与重试的相互作用、故障模式与常见误区。

> **前置阅读**：作用在[服务注册](../../components/service_registry/service_registry.md)发现到的实例列表上；哈希路由用[一致性哈希](../../components/consistent_hash/consistent_hash.md)；与[熔断](./circuit_breaker.md)/[超时重试](./timeout_and_retry.md)配合做故障转移。
...
负载均衡必须只把流量发给**健康**的后端，否则会把请求源源不断打到故障实例：

- **主动健康检查**：LB 周期性对后端做 HTTP/TCP 探测，失败达阈值即摘除，恢复后再拉回。
- **被动异常检测（outlier detection）**：根据真实请求的错误率/连续失败自动临时踢出异常实例（Envoy 的 outlier detection），比纯主动探测更快感知「假健康」。
- **与注册中心配合**：实例列表来自[服务注册](../../components/service_registry/service_registry.md)，LB 在其上叠加健康判定，双保险。

---

## 慢启动与预热

新上线/刚恢复的实例是「冷」的（连接池、JIT、本地缓存未热），若立刻按正常权重打满会超时甚至被压垮。**慢启动（slow start）**：新实例的权重在一段时间内从小逐步升到正常，给它预热时间——这与[服务注册](../../components/service_registry/service_registry.md)的上线预热一脉相承。

---

## 进阶算法：P2C 与 EWMA

- **P2C（Power of Two Choices）**：随机选两个后端，再从中选负载更低的那个。相比「全局最少连接」几乎零成本却能显著避免羊群，是现代 LB（如 gRPC、Finagle）常用算法。
- **EWMA 响应时间**：用指数加权移动平均平滑后端的实时延迟作为选路依据，比瞬时值稳定，能快速避开变慢的实例。
...
- **DNS/GSLB**：按地理位置、机房健康返回最优 IP，实现就近接入与机房级故障转移。
- **Anycast**：同一 IP 在多地宣告，网络层就近路由，常用于 CDN/DNS。
- 全局 LB 解决「用户到机房」的分配，机房内再由 L4/L7 LB 分配到实例，形成多级负载均衡。

---

## 与重试的相互作用

负载均衡与[重试](./timeout_and_retry.md)配合能做故障转移（一个实例失败换一个重试），但要防**重试放大**：所有实例都在过载时，盲目重试会雪上加霜。对策：重试预算、只对幂等操作重试、配合[熔断](./circuit_breaker.md)。
...
| 问题 | 说明 | 对策 |
| :--- | :--- | :--- |
| 流量打到故障实例 | 只发现不健康判定 | 主动健康检查 + 被动异常检测 |
| 冷实例超时 | 新实例立刻满载 | 慢启动预热 |
| 长连接负载不均 | 建连后连接粘住，扩容的新实例分不到流量 | 连接最大存活/定期重连、subset 重平衡 |
| 重试放大 | 过载时盲目换实例重试 | 重试预算 + 熔断 + 幂等 |
| 会话保持热点 | 粘性把某用户/大 key 固定到一台 | 一致性哈希 + 热点打散 |

- **误区：轮询就够了。** 后端异构或负载不均时轮询会造成热点，用最少连接/P2C/响应时间更稳。
- **误区：LB 只要会分流量。** 必须叠加健康检查与异常摘除，否则把请求打给死实例。
- **误区：新实例直接满载。** 需慢启动预热。
- **误区：长连接下扩容立即生效。** 已建连接粘住旧实例，新实例分不到流量，需重连/重平衡。
