...
- **误区：共用一个线程池没问题。** 一个慢依赖会占满共享线程，拖垮对其他依赖的调用；需按依赖分池隔离。
- **误区：线程池隔离和信号量隔离等价。** 线程池能超时中断、隔离更强但有上下文切换开销；信号量轻量但无法打断执行。
- **误区：非核心依赖不会影响核心。** 共享资源时非核心会拖垮核心，需资源池隔离 + 熔断非核心 + 降级。
- **误区：有了限流/熔断就不用隔离。** 限流控总量、熔断快速失败、隔离分资源，三者层次不同、互补。
- **误区：多租户共享资源无妨。** 一个租户突发会影响他人（吵闹邻居），需按租户/渠道分池。
