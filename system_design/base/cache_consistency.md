...
> **摘要**：缓存把热数据放到高速介质换取读性能，但只要「数据库」与「缓存」是两份副本，就必然存在一致性问题。本文建立一条主线：**为什么会不一致 → 三种读写模式 → Cache Aside 的更新顺序博弈 → 一致性增强手段 → 三大缓存故障（击穿/穿透/雪崩） → 多级缓存 → 治理指标与常见误区**，目标是读完能针对不同一致性诉求给出可落地的缓存方案。
...
核心认知：**缓存一致性追求的不是「永不脏」，而是「脏窗口足够短且可收敛」**。根本原因是「更新 DB」与「失效缓存」跨了两个无法共享本地事务的存储——这是一个**无分布式事务的最终一致问题**（呼应[分布式事务](./distributed_transaction.md)），因此只能把脏窗口压小并保证收敛，而非消除。真要强一致就不能用旁路缓存，而要读写都穿透缓存组件、写时加分布式锁串行化（性能代价大，仅少数场景值得）。
...
记忆锚点：**击穿是「一个热点」，穿透是「查空」，雪崩是「一片同时失效」**——三者成因不同，对策也不同，不要混用。
...
- **误区：更新缓存比删缓存好。** 并发写下两个更新可能乱序覆盖成脏；删缓存 + 懒加载既避免乱序又省去重算。
- **误区：先删缓存再写 DB 更安全。** 并发下会把旧值回填且不自愈；应先写 DB 再删缓存，脏窗口敏感再加延迟双删/binlog 订阅。
- **误区：删缓存成功就一致了。** 删缓存可能失败，需重试队列或 binlog 订阅统一删除兜底。
- **误区：缓存能做到强一致。** 旁路缓存本质是最终一致；要强一致须读写穿透 + 写锁串行化，代价大。
- **误区：本地缓存天然一致。** 多实例各自持有 L1，需短 TTL + 广播失效收敛。
- **误区：击穿/穿透/雪崩一个方案通吃。** 击穿用 singleflight/逻辑过期、穿透用空值+布隆、雪崩用 TTL 抖动+多级+高可用，对症下药。
