...
> **摘要**：微服务把单体拆成一组独立部署、围绕业务能力自治的小服务，换取独立演进与弹性，代价是分布式复杂度（服务治理、通信、数据一致性、可观测）。本文覆盖服务治理各面向，并补充拆分原则、单体 vs 微服务的取舍、常见误区与引用。

> **前置阅读**：微服务是[方法论](../设计可扩展分布式系统的方法论.md)中架构范式选择的落地；治理依赖[服务注册](../components/service_registry/service_registry.md)、[配置中心](../components/config_center/config_center.md)、[熔断](./high_availability/circuit_breaker.md)、[跨组件协同](../components/cross_component/cross_component.md)。

---

## 拆分原则与何时上微服务

- **先单体后微服务**：业务未定型、团队小时，单体（或模块化单体）迭代更快；出现「部署耦合、团队协作瓶颈、局部扩展困难」时再拆。**过早微服务化**会用分布式复杂度拖垮小团队。
- **拆分依据**：按**业务能力/限界上下文（DDD）**拆，而非按技术分层；一个服务应「高内聚、数据私有、可独立部署」。
- **数据私有**：每个服务独占自己的库，禁止跨服务直连他人库——否则又变回耦合的分布式单体。
...
- **误区：微服务一定比单体好。** 微服务用分布式复杂度换独立演进，小团队/未定型业务用单体更快；先单体后微服务。
- **误区：按技术分层拆服务。** 应按业务能力/限界上下文拆，按层拆会导致一个需求改动多个服务。
- **误区：多个服务共用一个库。** 数据必须私有，共库会变成「分布式单体」，失去独立部署与演进。
- **误区：拆得越细越好。** 过细导致跨服务调用暴增、事务与运维成本飙升，粒度要匹配团队与业务边界。
- **误区：微服务不用治理也能跑。** 注册发现、配置、熔断降级、链路追踪是微服务的必备控制面，缺了会失控。

---

## 引用关系

- 组件：[服务注册](../components/service_registry/service_registry.md)、[配置中心](../components/config_center/config_center.md)、[跨组件协同](../components/cross_component/cross_component.md)
- 机制：[熔断](./high_availability/circuit_breaker.md)、[降级](./high_availability/degradation.md)、[分布式事务](./distributed_transaction.md)、[服务身份](./security/service_identity.md)
- 上游：[设计可扩展分布式系统的方法论](../设计可扩展分布式系统的方法论.md)（单体 vs 微服务、DDD 拆分）
