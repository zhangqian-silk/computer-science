...
<template>
	<div class="infra-lab">
		<p class="infra-lab__title">Paged KV 容量模拟</p>
		<p class="infra-lab__hint">固定长度只是便于观察末 block 内部碎片；真实服务应输入长度分布。</p>
		<div class="infra-controls">
			<div class="infra-control"><label for="paged-length">每序列长度：{{ length }}</label><input id="paged-length" v-model.number="length" type="range" min="1" max="8192"></div>
			<div class="infra-control"><label for="paged-block">Block token</label><select id="paged-block" v-model.number="blockSize"><option :value="8">8</option><option :value="16">16</option><option :value="32">32</option><option :value="64">64</option></select></div>
			<div class="infra-control"><label for="paged-sequences">序列数：{{ sequences }}</label><input id="paged-sequences" v-model.number="sequences" type="range" min="1" max="64"></div>
			<div class="infra-control"><label for="paged-kv-bytes">KV KiB/token：{{ bytesPerTokenKiB }}</label><input id="paged-kv-bytes" v-model.number="bytesPerTokenKiB" type="range" min="8" max="256" step="8"></div>
		</div>
