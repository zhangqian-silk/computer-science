...
const cosine = computed(() => normA.value === 0 || normB.value === 0 ? 0 : dot.value / (normA.value * normB.value))
...
<template>
	<div class="infra-lab">
		<p class="infra-lab__title">向量相似度实验台</p>
		<p class="infra-lab__hint">改变两个二维向量，比较点积、余弦和欧氏距离分别保留什么信息。</p>
		<div class="infra-controls">
			<div class="infra-control"><label for="vector-ax">A.x：{{ ax.toFixed(2) }}</label><input id="vector-ax" v-model.number="ax" type="range" min="-1" max="1" step="0.05"></div>
			<div class="infra-control"><label for="vector-ay">A.y：{{ ay.toFixed(2) }}</label><input id="vector-ay" v-model.number="ay" type="range" min="-1" max="1" step="0.05"></div>
			<div class="infra-control"><label for="vector-bx">B.x：{{ bx.toFixed(2) }}</label><input id="vector-bx" v-model.number="bx" type="range" min="-1" max="1" step="0.05"></div>
			<div class="infra-control"><label for="vector-by">B.y：{{ by.toFixed(2) }}</label><input id="vector-by" v-model.number="by" type="range" min="-1" max="1" step="0.05"></div>
		</div>
		<svg class="vector-plane" viewBox="0 0 220 220" role="img" :aria-label="`向量 A 和 B，余弦相似度 ${cosine.toFixed(2)}`">
			<defs>
				<marker id="arrow-a" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="var(--vp-c-brand-1)" /></marker>
				<marker id="arrow-b" markerWidth="8" markerHeight="8" refX="7" refY="4" orient="auto"><path d="M0,0 L8,4 L0,8 Z" fill="var(--vp-c-green-1)" /></marker>
			</defs>
			<line x1="20" y1="110" x2="200" y2="110" class="axis" />
			<line x1="110" y1="20" x2="110" y2="200" class="axis" />
			<line x1="110" y1="110" :x2="pointA.x" :y2="pointA.y" class="vector vector-a" marker-end="url(#arrow-a)" />
			<line x1="110" y1="110" :x2="pointB.x" :y2="pointB.y" class="vector vector-b" marker-end="url(#arrow-b)" />
		...
		<div class="infra-results">
			...
			<div class="infra-result"><span>余弦相似度</span><strong>{{ cosine.toFixed(3) }}</strong></div>
