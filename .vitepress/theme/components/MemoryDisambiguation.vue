...
<template>
	<div class="mdp">
		...
		<p class="pc-note">
			六组的区分依据各不相同，这本身就是要点：<b>没有一条通用规则能同时辨清全部六组</b>。
			前两组靠「内容 vs 容量」和「因人而异 vs 对所有人一样」，中间两组靠「检索目标」和「载体 vs 内容类型」，
			后两组靠「隔离维度」和「是否绑定具体时间点」。真正可迁移的不是结论，而是每组末尾那句判别问法——
			它们都能在几秒内回答，并且不需要先记住任何术语定义。
		</p>
	</div>
</template>
