...
const resetLab = useLabReset(scores, temperature)
</script>

<template>
	<LearningLab topic="AttentionReadExplorer" @reset="resetLab">
		...
		<div class="infra-controls">
			<div v-for="(score, index) in scores" :key="index" class="infra-control">
				<label :for="fieldId(`attention-score-${index}`)">分数 {{ index + 1 }}：{{ score.value.toFixed(1) }}</label>
				<input :id="fieldId(`attention-score-${index}`)" v-model.number="score.value" type="range" min="-4" max="4" step="0.1">
			</div>
			<div class="infra-control">
				<label :for="fieldId('attention-temperature')">温度：{{ temperature.toFixed(1) }}</label>
				<input :id="fieldId('attention-temperature')" v-model.number="temperature" type="range" min="0.2" max="3" step="0.1">
			</div>
		</div>
