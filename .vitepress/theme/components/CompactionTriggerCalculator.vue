...
const verdict = computed(() => {
	...
	if (r < reserve.value * 0.6) return { tone: "fail", text: `只剩 ${fmt(r)} 供摘要输出，不足所需的 ${fmt(reserve.value)}——越线后可能连摘要请求都发不出去` }
...
const table = computed(() =>
	WINDOWS.map((w) => {
		...
		return {
			...
			p,
			left,
			state: left < reserve.value * 0.6 ? "fail" : left > reserve.value * 2.5 ? "warn" : "pass"
		}
	})
)
</script>
...
<style scoped>
.ctc { margin: 1rem 0; background: var(--cs-color-bg); border: 1px solid var(--cs-color-border); border-radius: var(--cs-radius-lg); padding: var(--cs-space-5) var(--cs-space-6); }
.ctc__controls { display: grid; grid-template-columns: repeat(2, 1fr); gap: var(--cs-space-3) var(--cs-space-5); }
.ctc__ctl { display: grid; grid-template-columns: 6.2rem 1fr 3.2rem; align-items: center; gap: var(--cs-space-2); min-height: var(--cs-tap-target); font-size: var(--cs-text-sm); }
...
.ctc__ctl input { width: 100%; accent-color: var(--cs-color-brand); }
.ctc__bar { position: relative; display: flex; height: 34px; margin: calc(var(--cs-space-6) + var(--cs-space-4)) 0 var(--cs-space-6); border: 1px solid var(--cs-color-border); border-radius: var(--cs-radius-sm); overflow: visible; }
...
.ctc__mark i { position: absolute; top: -6px; bottom: -6px; width: 2px; }
...
.ctc__mark--abs em { top: -24px; color: var(--cs-color-brand); }
.ctc__mark--pct i { background: var(--cs-color-danger); }
.ctc__mark--pct em { bottom: -24px; color: var(--cs-color-danger); }
...
@media (max-width: 720px) {
	...
	.ctc__ctl { grid-template-columns: 5.6rem 1fr 3rem; }
