...
保护前缀缓存的通用做法是<strong>把变更限制在已缓存内容之后</strong>：摘要作为稳定项放在消息链尾部，后续轮次只在其后追加。纯客户端若直接回写历史中间部分，前缀必然失效，只能通过攒够清理量或选择缓存过期时机来降低重写成本。
...
<div class="ctxc-grid ctxc-grid-2">
	<div class="ctxc-card ctxc-card-s"><h3>服务端 blob：摘要由服务端持有</h3><p>Codex 的服务端压缩把旧历史改写为一个不透明的 compaction blob，由服务端生成和存储，客户端只持有引用。再次压缩时旧 blob 被改写为新 blob，体积不随压缩次数累积。摘要放在消息链尾部以保持前缀命中——这一尾部策略客户端摘要同样可以采用；blob 的真正特点是不透明、不可审计，访问控制必须在服务端完成。</p></div>
	<div class="ctxc-card ctxc-card-w"><h3>cache_edits：不改字节，只改可见性</h3><p>Claude Code 在请求中附加编辑指令，告诉服务端屏蔽旧槽位。本地 messages 和服务端缓存的原始字节都不变，前缀缓存不被打碎，被屏蔽内容仍可恢复。代价是要求模型服务提供协议级可见性编辑能力，纯应用层客户端无法单独实现。</p></div>
</div>
