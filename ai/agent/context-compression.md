...
长任务 Agent 会把用户消息、工具调用、观察结果、推理痕迹和外部状态持续追加到历史中。上下文压缩要解决的不是「怎样把历史截短」，而是在下一次请求模型时决定：什么保持原文，什么只保留恢复句柄，什么在请求时投影，什么必须固化到窗口外。

因此，压缩的直接对象不是磁盘上的完整历史，而是**下一轮准备发给模型的那部分内容**。成熟系统通常同时保存完整事件、维护带标记的可用记录、临时组装本轮输入，并让工具返回值、持久记忆、读时投影、摘要和 KV Cache 策略协同工作。
...
更大的窗口只降低压缩频率，不取消上下文管理。能力边界应由[长上下文评估](../evaluation/long-context-evaluation.md)中的证据位置、任务类型和成本曲线确定，而不是只看名义窗口。
...
这意味着目标不是「尽量少给 token」，而是找到在当前任务阶段最能支持下一步决策的高信号集合。
...
源手册中的 <code>13K</code>、<code>20K</code>、<code>50K</code>、最近 3–5 个工具结果等数值，都是特定模型、产品、SDK 或社区解读下的配置。它们适合作为理解参数类别的例子，不应被抽象成通用默认值。

<details>
	<summary>示例参数类别（产品版本相关，不作为通用默认值）</summary>
	<p>工具结果清理线决定何时先清理可重取观察；最近窗口保护当前推理链；单次最少清理量用于判断是否值得打破缓存；摘要触发线为摘要输出和下一轮动作预留固定预算；硬阻塞线阻止继续发送。具体数值应来自目标模型的输出分布、计费口径和实测任务。</p>
</details>
...
压缩能减少后续输入 token，但修改历史会破坏前缀缓存。Agent 每轮通常只在尾部追加，系统提示、工具定义和早期消息高度重复；一旦改动历史中的某个位置，该位置之后的 KV Cache 都可能需要重新计算。底层缓存机制见 [KV Cache 管理](../infra/inference/kv-cache-management.md)。
