...
Context 是一次模型调用实际发送的 token 序列，在每次请求时重新构造；memory 与 knowledge 在窗口外持久化，经召回和装配进入本轮输入。三者的正式边界见 [Context、Memory 与 Knowledge 的边界](notes/context-boundary.md)。
...
更大的窗口只降低压缩频率，不取消上下文管理；[长上下文评估](../evaluation/long-context-evaluation.md)中的证据位置、任务类型和成本曲线决定能力边界，名义窗口不代表可用容量。
