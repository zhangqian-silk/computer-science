...
除了预先检索再填充，还可以采用即时检索：上下文只保留轻量标识符（文件路径、符号名、查询语句），运行时用工具按需加载正文。此时元数据本身就是信号——`tests/test_utils.py` 与 `src/core_logic/test_utils.py` 含义不同，目录层级、命名与时间戳都在提示该不该读、何时读。代价是运行时探索比预取慢，需要好的工具与启发式，否则会在死胡同里烧掉预算。实践中常用混合：项目说明类文件预先载入，其余靠 glob、grep 这类原语即时取用。
