...
Workflow 用程序或已确定的图组织模型调用、工具执行和普通计算。它适合路径可描述、步骤需要独立校验的任务，不是“低级 Agent”。本页讨论控制与数据依赖；模型驱动的下一动作选择见 [ReAct](react-loop.md)，模型生成编排程序见 [Dynamic Workflows](dynamic-workflows.md)。
