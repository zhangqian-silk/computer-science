...
| 可执行的工具与可加载的操作知识 | 前者提供能力与授权，后者只提供做法（详见 [Agent Skills](tools/skills.md)） |
| 写进提示词的约束与开在循环上的扩展点 | 前者是概率性的、失效时没有信号，后者是确定性的（详见 [Agent 扩展机制](tools/extensibility.md)） |
| 重试一次请求与重放一整条流 | 前者失败时尚未产出可原样重放，后者已产出一半、重放即重复（详见 [Agent 运行时的工程问题](runtime/engineering-problems.md)） |
| 恢复到检查点与重跑执行单元 | 前者重建视图后接着走，后者会让中断前的副作用再发生一次（详见 [Agent 运行时的工程问题](runtime/engineering-problems.md)） |
