...
这里要分清几种常被混为一谈的「记忆」：保存的消息历史、[本轮实际发送的上下文](./context-boundary.md)、结构化任务状态、长期记忆，以及 KV Cache。前几者处理信息的保存与使用，KV Cache 处理模型计算的复用，两者相互独立。历史会随轮次增长，因此实际发送时通常要做选择、裁剪或摘要。
...
工具执行前的参数校验、授权与确认，通过循环上预留的扩展点接入；这些挂载位置、返回值契约与多个扩展的组合方式见[扩展机制](./extension-mechanisms.md)。
...
| 可执行的工具与可加载的操作知识 | 前者提供能力与授权，后者只提供做法 |
| 写进提示词的约束与开在循环上的扩展点 | 前者是概率性的、失效时没有信号，后者是确定性的 |
| 重试一次请求与重放一整条流 | 前者失败时尚未产出可原样重放，后者已产出一半、重放即重复 |
| 恢复到检查点与重跑执行单元 | 前者重建视图后接着走，后者会让中断前的副作用再发生一次 |
