...
Tool Calling 让模型提出结构化动作，工具网关将该提议转换成受校验、受授权的实际操作。它可用于一次调用后直接返回，也可用于 Workflow 或 Agent Loop。本页完整解释执行链。
...
因此安全策略应落在实际资源接口和执行环境。允许 `bash` 通常比允许一个受限 `read_file` 大得多；隐藏危险工具不代表其依赖进程无法完成相同行为。
