...
Fleet 管理的是一组可运行的模型副本，而不只是空闲 GPU 数。一个 TP group 必须同时得到兼容设备与互联，模型加载和预热又形成分钟级或更长的准备时标；因此放得下、启动得及和满足 SLO 是三种不同容量。

---

## 两层容量

先测一个 replica group 在指定长度分布和延迟阈值下的 Goodput，再推算 group 数，最后映射到实际节点。公式结果必须向上取整；还要检查单节点故障是否同时带走多个 group，以及发布期间新旧版本是否需要额外空间。

利用率余量不能重复计算：若 Goodput 已是在保守负载下测得，再乘另一保守因子时要说明各自保护的风险。平均需求公式也不能替代突发流量和最长输出的容量检验。
...
若从扩容信号到可接流量需 5 分钟，而突发持续 30 秒，反应式扩容无法服务这次突发本身。可能的选择是预热、预扩容、有界排队或明确拒绝；哪个合理由 SLO 与成本决定，不能靠更频繁轮询弥补物理加载时间。
