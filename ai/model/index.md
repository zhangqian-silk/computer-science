...
模型页将表示、机制、目标和推理接口组合成完整计算。下面按上下文载体与任务接口交叉组织，而非互斥分类：Seq2Seq 可以使用循环、Attention 或其他主干，BERT/GPT 可以共享 Transformer 机制却采用不同目标。先读[分类地图](../foundations/learning-problem-map.md)，再沿具体路线深入。
...
| [GPT](./gpt.md) | 输入位置的状态预测后一 token，使用因果 mask | Prompt、逐步生成结果与各层 KV cache |
...
每篇读完应能恢复四件事：一条训练样本怎样构造、前向产生哪些状态、loss 更新哪些参数、推理保留什么。再沿参考论文区分原始版本与后续改造；发布时间提供线索，但不能代替结构与监督关系。
