...
本文主要负责说明 Transformer 作为完整架构时的主干逻辑，包括 block 结构、三类结构路线、训练推理骨架与历史位置。attention、自注意力、位置机制的完整数学细节分别以 [Attention](../mechanism/attention.md)、[Self-Attention](../mechanism/self-attention.md) 与 [Positional Encoding](../mechanism/positional-encoding.md) 为主文档；高效变体、长上下文、ViT 与多模态扩展则统一收拢到 [transformer-extensions.md](./transformer-extensions.md)。
