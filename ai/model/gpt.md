...
为便于统一表述，本文主要采用现代 GPT 类模型中更常见的 **pre-LN** 写法展开，同时以 **masked self-attention + FFN + residual + LayerNorm** 作为基本分析骨架。原始 GPT、GPT-2 / GPT-3 与后续 LLaMA 类模型在位置编码、激活函数、是否共享输出层权重等实现细节上并不完全相同，但共享同一条数学主线。

---

## 路线图与符号约定

若将全文压缩为一条主线，则 GPT 的内部逻辑可以概括为：
...
## 与 BERT 和 Encoder-Decoder 路线的区别
